...
import { Group, GroupsCollection } from "@bundles/UIAppBundle/collections";
...
export class GroupsAntTableSmart extends AntTableSmart<Group> {
  collectionClass = GroupsCollection;

  getBody(): QueryBodyType<Group> {
    return GroupList.getRequestBody();
  }

  getColumns(): ColumnsType<Group> {
    const list = this.container.get(GroupList);
    list.build();

    return [...list.rest(), this.getActionsColumn()];
  }

  getActionsColumn(): ColumnType<Group> {
    return {
  ...
  getActionItems() {
    ...
    if (features.view) {
      actions.push({
        ...
        icon: <EyeOutlined />,
        action: (model) => {
          this.router.go(Routes.GROUPS_VIEW, {
    ...
    if (features.edit) {
      actions.push({
        ...
        icon: <EditOutlined />,
        action: (model) => {
          this.router.go(Routes.GROUPS_EDIT, {
            params: { id: model._id.toString() },
          });
        },
      });
